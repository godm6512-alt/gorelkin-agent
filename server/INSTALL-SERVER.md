...
> Сейчас нужно авторизовать Claude на сервере. Это нужно потому, что бот будет разговаривать через Вашу подписку Claude — за ответы платит подписка, никаких API-ключей и доплат сверху не нужно. Авторизация делается один раз — как войти в аккаунт.
>
...
> ```
> **Шаг 4.** На экране появится длинная ссылка и код. Скопируйте ссылку, откройте её в браузере на компьютере → нажмите **Authorize**. На сайте появится код — нажмите **Copy**.
> **Шаг 5.** Вернитесь в консоль сервера и вставьте код. Внимание: при вставке Вы **не увидите** код в терминале — это нормально, так специально сделано для безопасности (как с паролем). Просто вставьте один раз и нажмите Enter.
> **Шаг 6.** Должна появиться строчка вида «Logged in as ...» — это значит всё получилось.
>
> Когда закончите, напишите мне «готово».
>
> **Если что-то пошло не так:** сделайте скриншот консоли и пришлите мне с пояснением где Вы — я посмотрю и подскажу. Самые частые случаи: код просрочился (повторите команду заново), или код вставили два раза подряд (вставляйте ровно один раз и сразу Enter).
...
> Остался последний шаг — настроить **туннель** к серверу.
>
> **Зачем это нужно.** Туннель — это прямой коридор между Вашим VS Code на компьютере и сервером. Один раз настроили — и сервер всегда у Вас в списке: открываете VS Code → видите файлы Агента, правите их как обычную папку. Без туннеля файлы пришлось бы править через чёрное окно консоли в браузере — неудобно и медленно.
>
...
> ```
> **Что произойдёт после команды:** на экране появится ссылка вида `https://github.com/login/device` и 8-значный код (например `ABCD-1234`). Это GitHub просит разрешить туннелю работать через Ваш аккаунт — туннель привязывается к GitHub, чтобы только Вы могли подключаться к своему серверу.
>
> **Шаг 8.** Откройте эту ссылку в браузере на вашем компьютере
> **Шаг 9.** Введите код с экрана консоли и нажмите **«Authorize»**
> **Шаг 10.** Вернитесь в консоль Beget и нажмите **Ctrl+C** (зажмите Ctrl и нажмите букву C)
> **Шаг 11.** Введите последнюю команду и нажмите Enter:
> ```
...
## Шаг 10. Шпаргалка после установки

Когда все автоматические шаги прошли, выведи пользователю такой итог. **Это важный финальный экран** — он закрывает установку и показывает что осталось вручную.
...
Что готово ✓
  ...
  — Файлы Агента скопированы в /home/agent/workspace/
  — Telegram-бот установлен и запущен (если давали токен)

Что осталось вручную (2 коротких шага, ~5 минут)
  ☐ 1. Авторизовать Claude — чтобы бот мог разговаривать через Вашу подписку
       Как: вернитесь к Шагу 7.4 этой инструкции, идите по нему
  ☐ 2. Настроить VS Code Tunnel — чтобы редактировать файлы Агента из своего VS Code
       Как: вернитесь к Шагу 8 этой инструкции, идите по нему

Как проверить что всё работает
  — Telegram-бот: напишите боту /start → он проведёт 6 вопросов
    и создаст ДНК агента автоматически
  — VS Code Tunnel: откройте VS Code → Remote Explorer → Tunnels →
